...
def getData(url):
    ...
    for i, data in enumerate(result):
        ...
        sqlinsert = ("INSERT INTO page_data(WebName,Link,house,adress,money,house_type,floor,square_meters,pattern)" "VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)")
        val = ['樂屋網',data['Link'],data['house_name'],data['address'],data['house_money'],data['house_type'],data['floor'],data['square_meters'],data['pattern']]
        cursor.execute(sqlinsert,val)
        db.commit()
    print(f'Total: {len(result)}')
    return soup.find(class_="pages").string[5:-1]
